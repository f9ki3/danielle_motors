...
if (isset($_POST['transaction_code'])) {
    ...
    $stmt_purchase_transactions = $conn->prepare("UPDATE purchase_transactions SET status = 5 WHERE TransactionID = ?");
    ...
    if ($stmt_purchase_transactions->execute()) {
        ...
        $stmt_purchase_cart = $conn->prepare("UPDATE purchase_cart SET status = 5 WHERE TransactionID = ?");
